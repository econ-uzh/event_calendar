...
module EventCalendar
  ...
  module ClassMethods
    def has_event_calendar(options = {})
      ...
      unless start_at_field == 'start_at'
        alias_attribute :start_at, start_at_field
      end
    ...
    def events_for_date_range(start_d, end_d, find_options = {})
      find_options[:conditions][0] = "#{unless find_options[:conditions][0].blank?
                                          "(#{find_options[:conditions][0]}) AND "
                                        end}? <= #{quoted_table_name}.#{end_at_field} AND #{quoted_table_name}.#{start_at_field} < ?"

      where(find_options[:conditions][0], *find_options[:conditions][1], start_d.to_time.utc, end_d.to_time.utc).joins(find_options[:include]).includes(find_options[:include]).order("#{quoted_table_name}.#{start_at_field} ASC")
    end
    ...
    def create_event_strips(strip_start, strip_end, events)
      ...
      events.each do |event|
        ...
        next unless (start_range <= end_range) && (end_range >= 0)
  ...
  module InstanceMethods
    ...
    def clip_range(start_d, end_d)
      ...
      clipped_start = if (start_at_d < start_d) && (end_at_d >= start_d)
                        start_d
                      else
                        start_at_d
                      end
    ...
    def adjust_all_day_dates
      if all_day
        ...
        self.end_at = if end_at
                        end_at.beginning_of_day + 1.day - 1.second
                      else
                        start_at + 1.day - 1.second
                      end
      end
    end
  end
